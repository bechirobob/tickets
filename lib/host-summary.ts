...
export async function readHostSummary(db: D1Database, slug: string) {
  ...
  const [rsvp,sales,interest,sources,pending] = await Promise.all([
    ...
    db.prepare(`SELECT COUNT(*) AS orders,COALESCE(SUM(face_amount_minor),0) AS ticketSalesMinor,
      ...
      FROM orders o WHERE o.event_slug=? AND o.status='paid' AND o.payment_provider<>'rsvp' AND COALESCE(o.paid_at,o.created_at)>=?`).bind(slug,start).first<{orders:number;ticketSalesMinor:number;admissions:number;checkedIn:number}>(),
    db.prepare(`SELECT (SELECT COUNT(*) FROM event_registrations WHERE event_slug=? AND kind='interest' AND status='interested' AND created_at>=?) AS count,
      ...
      (SELECT COALESCE(SUM(admission_count),0) FROM guest_entries WHERE event_slug=? AND created_by<>'system:rsvp' AND id NOT LIKE 'rsvp:%' AND status='checked_in' AND created_at>=?) AS manualArrivals`).bind(slug,start,slug,start,slug,start).first<{count:number;manualGuests:number;manualArrivals:number}>(),
    db.prepare(`SELECT COALESCE(promoter_code,'') AS code,COUNT(*) AS orders FROM orders WHERE event_slug=? AND status='paid' AND payment_provider<>'rsvp' AND COALESCE(paid_at,created_at)>=? GROUP BY promoter_code ORDER BY orders DESC,code LIMIT 5`).bind(slug,start).all<{code:string;orders:number}>(),
