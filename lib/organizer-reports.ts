...
export function hostReportEmail(name:string,kind:'weekly'|'recap',summaries:HostSummary[],asOf:string) {
  ...
  const rows=summaries.map(s=>{
    ...
    const stats=[...(s.baseline?[`Analytics since ${new Date(s.baseline).toLocaleString('en-GB',{timeZone:'Africa/Accra'})} (Accra time)`]:[]),`${s.rsvp.totals.requests} RSVP requests · ${s.rsvp.totals.confirmedGuests} confirmed RSVP guests`,`${s.pending} requests waiting for your approval`,`${s.interest} people on the email list (not admissions)`,`${s.sales.orders} paid orders · ${money(s.sales.ticketSalesMinor)} ticket sales`,`${s.checkedIn} of ${s.expected} expected guests checked in${s.turnoutPercent===null?'':` · ${s.turnoutPercent}% turnout`}`];
  ...
  const note='Totals cover activity since the analytics start shown above, or the whole event where no reset is recorded. Pending approvals always show the current queue. Ticket sales exclude booking fees and are not a payout statement. Cancelled and refunded orders are excluded; turnout uses recorded arrivals. Source counts describe tracked links, not proof that a post caused a booking.';
...
export async function reportDeliveryAllowed(db:D1Database,id:string|null,recipient:string,now=new Date().toISOString()) {
  const report=await db.prepare(`SELECT r.event_slugs_json AS slugs FROM organizer_reports r JOIN staff_accounts a ON a.id=r.account_id
    ...
    AND r.created_at>=COALESCE((SELECT started_at FROM analytics_baseline WHERE id=1),'1970-01-01')
