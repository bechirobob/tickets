...
export function marketingAudienceSql(group:MarketingAudience) {
 ...
 return `SELECT a.id,a.email FROM event_audience_contacts a LEFT JOIN marketing_contacts m ON m.email=a.email JOIN curated_event_records e ON e.slug=a.event_slug WHERE a.event_slug=? AND e.removed_at IS NULL AND ${marketingConsent} ${status}`;
}
