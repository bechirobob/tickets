...
The follow-up candidate passed the delayed-hydration navigation regression in all three engines. Mobile WebKit also exposed an independent RSVP tab interaction while the live activity panel above it was still expanding. Registration tabs now sit above that asynchronous panel, keeping the controls in place. A delayed-activity regression holds the pointer down while the live panel arrives, checks that the target has not moved and verifies that releasing opens Announcements. The original announcement failure/draft-preservation assertions remain intact.

Candidate `a4bc85b42838549839dbd5979dc15cd527ef0bc7` passed the navigation and live-activity interaction regressions in all engines. Its WebKit accessibility audit exposed a horizontally scrollable orders table without a keyboard focus target; the check passed on retry once inner actions became available. Orders, guest-email results and the capped organiser-activity list now have explicit keyboard focus and visible focus rings, so scrolling does not depend on inner links or loading timing. Existing expanded-screen accessibility gates remain unchanged.

The next WebKit run cleared the scroll-region accessibility finding but reproduced a lost first guest-email tab click with Setup still active. The workspaces inherited global animated document scrolling, which can move targets as focus changes. Workspace scrolling is now immediate. A repeated fresh-page regression exercises both guest-email and announcement tabs after scrolling away from them in owner and organiser workspaces; first-failure traces are retained for Operations checks. No assertion or retry budget is relaxed.
