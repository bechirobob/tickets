...
The approved public template, logo and compact flier are retained here. Recipient addresses and phone numbers do not belong in this public repository. The workflow resolves the exact recipient fingerprint against an active owner record in the private database and sends only one email. It does not add an RSVP or subscribe anyone to marketing.
