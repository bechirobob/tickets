...
const url=new URL(original.redirect_url);
if(url.origin!=='https://tickets.becoreops.com'||url.pathname!=='/payment/return'||url.searchParams.get('reference')!==order.reference||createHash('sha256').update(url.searchParams.get('claim')??'').digest('hex')!==order.token_hash)throw new Error('Original callback does not match its stored access grant.');
