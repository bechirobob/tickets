...
execFileSync("npx", ["wrangler", "d1", "execute", "DB", "--local", "--persist-to", ".wrangler/state", "--command",
  `UPDATE curated_event_records SET starts_at = '${start}', ends_at = '${end}', sales_open_at = NULL, sales_close_at = '${start}', is_test_event = 1, status = 'published', event_state = 'on_sale' WHERE slug = 'after-dark-osu'; UPDATE event_ticket_tiers SET status = 'available', sales_open_at = NULL, sales_close_at = '${start}' WHERE event_slug = 'after-dark-osu';`], { stdio: "inherit" });
