...
const remaining=await query(`SELECT slug FROM curated_event_records WHERE slug IN ('after-dark-osu','noir-room-labone','longitude-spintex') UNION ALL SELECT 'test-order' FROM orders WHERE payment_environment IN ('test','sandbox') OR id='88ad9bcd-2c2c-49f9-8115-ec982bd9a3c4'`);
if(remaining.length)throw new Error('Preview records remain after cleanup.');
const real=await query("SELECT slug,status,removed_at AS removedAt FROM curated_event_records WHERE slug IN ('the-weekend-braai','sun-chasers-labadi') ORDER BY slug");
// A later owner removal leaves an unpublished tombstone; never restore it during cleanup.
if(real.length!==2||real.some(row=>row.removedAt?row.status!=='unpublished':row.status!=='published'))throw new Error('Real event listing verification failed.');
