...
describe('RSVP admission and interest registrations', () => {
  ...
  it('requires assigned host approval and excludes anonymous roster access', async () => {
    ...
    const before = await passes(req('/api/customer/tickets', {}, claim.cookie));
    expect((await before.json() as { orders: unknown[] }).orders).toHaveLength(0);
    ...
    // Rehearse the complete approval journey using the original guest session.
    ...
    const walletResponse = await passes(req('/api/customer/tickets', {}, claim.cookie));
    expect(walletResponse.status).toBe(200);
    const wallet = await walletResponse.json() as { orders: Array<{ tickets: Array<{ qrPayload: string }> }> };
    expect(wallet.orders).toHaveLength(1);
    expect(wallet.orders[0].tickets).toHaveLength(1);
    const code = wallet.orders[0].tickets[0].qrPayload;
    const gateCookie = await owner();
    const scan = () => checkIn(req('/api/admin/check-in', { code, eventSlug: slug, gate: 'Rehearsal' }, gateCookie));
    expect((await scan()).status).toBe(200);
    expect((await scan()).status).toBe(409);
  });
