...
test("notification failures preserve unread items and retry recovers the inbox", async ({ page }) => {
  ...
  await expect(page.locator(".buzz-list > .buzz-row.unread")).toContainText(item.body);
  await expect(page.locator(".buzz-list .buzz-destination")).toHaveAttribute("href", item.url);
  ...
  await expect(page.locator(".buzz-list > .buzz-row.unread")).toHaveCount(1);
  await expect(mark).toBeEnabled();
  await mark.click();
  await expect(page.locator(".buzz-list > .buzz-row.read")).toHaveCount(1);
...
test("My Nights recovers from a service failure and keeps member navigation consistent", async ({ page }) => {
  ...
  if (mobile) await expect(page.getByRole("navigation", { name: "Customer navigation" }).getByRole("link", { name: "My Nights" })).toHaveAttribute("aria-current", "page");
