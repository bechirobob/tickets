...
test.beforeEach(async ({ page }) => {
  ...
  await page.route("**/api/customer/experience/the-weekend-braai", route => route.fulfill({ json: {
  ...
  await page.route("**/api/customer/tickets", route => route.fulfill({ json: {
    attendee: { attendeeId: "isolated-nights-review" }, orders: [{
      roomAccess: true, orderId: "isolated-order", reference: "BECORE-FIXTURE", eventSlug: "the-weekend-braai",
...
test("the pass leads, supporting tools expand, and the plan preserves answers", async ({ page }, info) => {
  await page.goto("/my-nights/the-weekend-braai");
  const nav = page.getByRole("navigation", { name: "Night views" });
  await expect(nav.getByRole("button")).toHaveCount(2);
  await expect(nav.getByRole("link", { name: "Room", exact: true })).toHaveAttribute("href", "/room/the-weekend-braai");
...
test("existing receipt and perks links still open their content", async ({ page }) => {
  await page.goto("/my-nights/the-weekend-braai?view=purchase");
  await expect(page.getByText("BECORE-FIXTURE", { exact: true })).toBeVisible();
  await page.goto("/my-nights/the-weekend-braai?view=perks");
  await expect(page.getByText("Grills, drinks and an afternoon with your people.")).toBeVisible();
