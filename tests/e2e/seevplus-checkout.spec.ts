...
test("SeevPlus stays compact and sends the selected provider without exposing credentials", async ({ page }) => {
  ...
  await expect(page.locator("#checkout-payment-message")).toHaveText("Choose a payment method before continuing.");
  ...
  await expect(page.locator("#checkout-payment-message")).toHaveText("Tick the ticket terms, refund rules and privacy checkbox to continue.");
  ...
  await page.getByRole("button", { name: /Pay with MoMo/ }).click();
  await expect(page.locator("#checkout-payment-message")).toHaveText("Add a valid email for your ticket and receipt.");
