...
import { ArrowLeft, Bell, CalendarDays, Check, CircleDollarSign, Clock3, Crown, Download, ExternalLink, Loader2, LockKeyhole, MapPin, MessageCircle, QrCode, ReceiptText, Save, ShieldCheck, Sparkles, Ticket, Users } from "lucide-react";
...
type TicketOrder = {
  ...
  currency: string; paidAt: string | null; bookedFor: string | null; tierName: string | null; tierDescription: string | null; tickets: GateTicket[];
};
...
export default function NightHub({ event }: { event: EventSummary }) {
  ...
  return <main className="night-hub">
    ...
    <section className="night-hub__view">
      ...
      {view === "passes" ? <div className="night-passes"><header><p className="eyebrow">Gate access</p><h2>Your moving tickets.</h2><p>Open this at the gate with your brightness up. The codes refresh; screenshots age badly.</p></header><div>{tickets.map((ticket, index) => <article key={ticket.id}><span>Ticket {index + 1}</span><b>{humanTicket(ticket.ticketType)}</b>{ticket.qrPayload && ticket.gateCode ? <><QrPass payload={ticket.qrPayload} label={`Entry QR code for ticket ${index + 1}`} /><code>{ticket.gateCode}</code></> : <p>{ticket.status === "checked_in" ? "Already inside. Excellent." : "This ticket is taking a moment."}</p>}</article>)}</div></div> : null}
      ...
      {view === "purchase" ? <div className="night-purchase"><header><p className="eyebrow">Receipts &amp; support</p><h2>The money trail, neatly behaved.</h2><p>Every amount below comes from the confirmed paid order. Accounting gets to be boring here.</p></header>{orders.map((order) => <article key={order.orderId}><header><div><CircleDollarSign /><span><b>{order.tierName ?? humanTicket(order.tickets[0]?.ticketType ?? "Admission")}</b><small>{order.reference}</small></span></div><button type="button" onClick={() => window.print()}><Download size={14} /> Print / save</button></header><dl><div><dt>Ticket subtotal</dt><dd>{money(order.faceAmountMinor, order.currency)}</dd></div><div><dt>Booking fee</dt><dd>{money(order.bookingFeeMinor, order.currency)}</dd></div><div><dt>Total paid</dt><dd>{money(order.totalAmountMinor, order.currency)}</dd></div><div><dt>Confirmed</dt><dd>{order.paidAt ? new Intl.DateTimeFormat("en-GH", { dateStyle: "medium", timeStyle: "short", timeZone: "Africa/Accra" }).format(new Date(order.paidAt)) : "Payment confirmed"}</dd></div></dl></article>)}<p className="night-purchase__support">Something genuinely off? <Link href="mailto:tickets@tickets.becoreops.com">Tell ticket support</Link>. “My friend changed their mind” remains a group-chat matter.</p></div> : null}
    </section>
