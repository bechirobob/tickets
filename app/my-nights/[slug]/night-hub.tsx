...
export default function NightHub({ event }: { event: EventSummary }) {
  ...
  async function save(input: { attendeeVisible?: boolean; keepPosted?: boolean; includeAnswers?: boolean }) {
    ...
    if (response.ok) {
      ...
      setNotice(input.includeAnswers ? "Answers saved. The Host can stop guessing now." : "Preference saved. Boundaries looking excellent.");
    }
  ...
  return <main className="night-hub">
    ...
    <section className="night-hub__view">
      ...
      {view === "overview" ? <div className="night-overview"><article><p className="eyebrow">Your access</p><h2>One ticket. The whole night.</h2><div><Link href={`/room/${event.slug}`}><MessageCircle /> <span><b>The Room</b>Chat, Host updates and Flashes—one conversation</span></Link><Link href={`/room/${event.slug}`}><Camera /> <span><b>Flashes</b>Temporary photos, dropped right into The Room</span></Link><button type="button" onClick={() => setView("passes")}><QrCode /> <span><b>Your passes</b>Moving QR codes for the gate</span></button></div></article><aside><p className="eyebrow">I&apos;m in</p><h3>Choose whether other ticket holders can count you in.</h3><p>Your name stays private. Turning this on adds one to the count—not your biography.</p><button type="button" onClick={() => save({ attendeeVisible: !experience.preference.attendeeVisible })} disabled={saving}>{experience.preference.attendeeVisible ? <Check size={15} /> : <Users size={15} />}{experience.preference.attendeeVisible ? "You’re visible as going" : "Count me in, quietly"}</button><span>{experience.visibleAttendees} people currently visible</span></aside></div> : null}
      {view === "passes" ? <div className="night-passes"><header><p className="eyebrow">Gate access</p><h2>Your moving passes</h2><p>Open this at the gate with your brightness up. The codes refresh; screenshots age badly.</p></header><div>{tickets.map((ticket, index) => <article key={ticket.id}><span>Pass {index + 1}</span><b>{ticket.ticketType.replaceAll("-", " ")}</b>{ticket.qrPayload && ticket.gateCode ? <><QrPass payload={ticket.qrPayload} label={`Entry QR code for pass ${index + 1}`} /><code>{ticket.gateCode}</code></> : <p>{ticket.status === "checked_in" ? "Already inside. Excellent." : "This pass is taking a moment."}</p>}</article>)}</div></div> : null}
