...
export default function MyNightsClient() {
  ...
  return <main className="my-nights-page">
    ...
    <section className="my-nights-shell">
      <header><div><p className="eyebrow">Ticket-earned access</p><h1>{loading ? "Opening My Nights…" : payload ? `${payload.attendee.displayName}’s nights.` : "My Nights starts with a ticket."}</h1></div>{payload ? <span><LockKeyhole size={13} /> Member access active</span> : null}</header>
      {loading ? <div className="my-nights-loading"><Loader2 className="spin" /> Preparing your nights</div> : locked ? <section className="my-nights-locked"><Ticket size={30} /><h2>Your first verified ticket opens this side of BeCore.</h2><p>Anyone can browse The Drop. My Nights, Host follows, Before the Night and private event access begin after a completed purchase.</p><div><Link href="/events">Browse The Drop</Link><Link href="/tickets">Recover paid tickets</Link></div></section> : <>
