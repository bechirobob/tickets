...
export default function TicketWallet() {
  ...
  return <main className="wallet-page wallet-page--real">
    ...
    <section>
      ...
      {wallet && !wallet.attendee.emailVerified ? <div className="wallet-confirmed wallet-confirmed--verify"><Mail size={21} /><div><b>This booking is ready</b><span>Bringing back older tickets? Use the link in your email.</span></div></div> : null}
      <Ticket size={38} /><p className="eyebrow">Your ticket wallet</p><h1>{loading ? "Getting your tickets…" : wallet ? `Good to see you, ${wallet.attendee.displayName}.` : "Your tickets are here."}</h1>
      {error ? <p className="wallet-error" role="alert">{error}</p> : null}
      {!loading && !wallet && <div className="wallet-locked"><BadgeCheck /><h2>{recoveryInvalid ? "That link already did its one job—or took too long getting dressed." : "Your wallet is waiting for a paid ticket."}</h2><p>Already paid? Use your booking email to bring your tickets back.</p><form className="wallet-recovery" onSubmit={requestRecovery}><label>Email used at checkout<input type="email" required autoComplete="email" value={recoveryEmail} onChange={(event) => setRecoveryEmail(event.target.value)} placeholder="you@example.com" /></label><button disabled={recoveryState !== "idle"}>{recoveryState === "sending" ? <Loader2 className="spin" size={15} /> : <Mail size={15} />} {recoveryState === "sent" ? "Check your email" : recoveryState === "sending" ? "Sending…" : "Email secure access link"}</button></form>{recoveryState === "sent" ? <small>If that email has tickets, a fresh access link is on its way.</small> : null}<Link href="/#drop">Choose a night</Link></div>}
