...
export default function RegistrationForm({ eventSlug, mode, maxPartySize, approvalRequired, initiallyOpen=false, deadline }: { eventSlug: string; mode: 'rsvp' | 'interest'; maxPartySize: number; approvalRequired: boolean; initiallyOpen?: boolean; deadline?: string }) {
  ...
  return <section className="registration-form">
    ...
    {!open ? <ActionButton disabled={runtime.stale} onClick={() => { if (runtime.openSecurePage) void runtime.openSecurePage(`/rsvp/${eventSlug}${window.location.search}`); else setOpen(true); }}>{mode === 'interest' ? 'Keep me posted' : approvalRequired ? 'Request an RSVP' : 'RSVP'}</ActionButton> : <form onSubmit={submit}>
      ...
      {!sent ? <fieldset disabled={busy}><label>Your name<input name="guestName" autoComplete="name" required minLength={2} maxLength={120} /></label><label>Email address<input name="email" type="email" autoComplete="email" required maxLength={254} /></label>{mode === 'rsvp' ? <><label>Phone (optional)<input name="phone" type="tel" autoComplete="tel" maxLength={40} /></label>{maxPartySize > 1 ? <label>Guests, including you<select name="partySize">{Array.from({ length: maxPartySize }, (_, i) => <option key={i} value={i + 1}>{i + 1}</option>)}</select></label> : null}</> : null}<label className="registration-consent"><input name="announcementsOptIn" type="checkbox" defaultChecked={mode === 'interest'} /><span>Receive notifications for this event and host.</span></label><label className="registration-consent"><input name="acceptedTerms" type="checkbox" required /><span>I accept the <Link href="/terms#purchase" target="_blank">event terms</Link> and <Link href="/privacy" target="_blank">privacy notice</Link>.</span></label><ActionButton type="submit" disabled={busy}>{busy ? 'Sending…' : mode === 'rsvp' ? 'Send RSVP' : 'Send my confirmation link'}</ActionButton></fieldset> : null}
