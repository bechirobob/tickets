...
export default function RegistrationForm({ eventSlug, mode, maxPartySize, approvalRequired, initiallyOpen=false, deadline }: { eventSlug: string; mode: 'rsvp' | 'interest'; maxPartySize: number; approvalRequired: boolean; initiallyOpen?: boolean; deadline?: string }) {
  ...
  return <section className="registration-form">
    {!open ? <ActionButton onClick={() => setOpen(true)}>{mode === 'interest' ? 'Keep me posted' : approvalRequired ? 'Request an RSVP' : 'RSVP'}</ActionButton> : <form onSubmit={submit}>
      {!sent ? <>{mode==='interest'?<p>Email updates · no admission included</p>:null}<h2>{mode === 'interest' ? 'Be first to hear' : 'Your RSVP'}</h2>
      <p>{mode === 'interest' ? 'We’ll email when the date or booking details change. This does not reserve admission.' : approvalRequired ? 'The host reviews requests. Your place is reserved only after confirmation.' : 'Leave your details. Places are limited; once full, new RSVPs join the waitlist.'}</p>
      ...
      {sent && mode === 'rsvp' ? <div className="registration-success" role="status"><span aria-hidden="true">✓</span><h2>RSVP received.</h2><p>Outfit planning starts now.</p>{approvalRequired ? <p className="registration-success__detail">Your request is with the host for approval.</p> : <p className="registration-success__detail">Your details are with the host. If the guest list is full, your RSVP joins the waitlist.</p>}</div> : <>{message ? <p role="status">{message}</p> : null}{sent ? <p>Check your inbox and spam folder. The link lasts 20 minutes.</p> : null}</>}
