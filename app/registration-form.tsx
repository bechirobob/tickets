...
export default function RegistrationForm({ eventSlug, mode, maxPartySize, approvalRequired }: { eventSlug: string; mode: 'rsvp' | 'interest'; maxPartySize: number; approvalRequired: boolean }) {
  const [open, setOpen] = useState(false), [busy, setBusy] = useState(false), [message, setMessage] = useState(''), [sent, setSent] = useState(false);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/registrations', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ eventSlug, email: form.get('email'), guestName: form.get('guestName'), phone: form.get('phone') ?? '', partySize: Number(form.get('partySize') ?? 1), acceptedTerms: form.get('acceptedTerms') === 'on' }) });
  ...
  return <section className="registration-form">
    {!open ? <ActionButton onClick={() => setOpen(true)}>{mode === 'interest' ? 'Keep me posted' : approvalRequired ? 'Request an RSVP' : 'RSVP — free entry'}</ActionButton> : <form onSubmit={submit}>
      <h2>{mode === 'interest' ? 'Be first to hear' : 'Save your place'}</h2>
      ...
      {!sent ? <fieldset disabled={busy}><label>Your name<input name="guestName" autoComplete="name" required minLength={2} maxLength={120} /></label><label>Email address<input name="email" type="email" autoComplete="email" required maxLength={254} /></label>{mode === 'rsvp' ? <><label>Phone (optional)<input name="phone" type="tel" autoComplete="tel" maxLength={40} /></label>{maxPartySize > 1 ? <label>Guests, including you<select name="partySize">{Array.from({ length: maxPartySize }, (_, i) => <option key={i} value={i + 1}>{i + 1}</option>)}</select></label> : null}</> : null}<label className="registration-consent"><input name="acceptedTerms" type="checkbox" required /><span>I accept the <Link href="/terms#purchase" target="_blank">event terms</Link> and <Link href="/privacy" target="_blank">privacy notice</Link>.</span></label><ActionButton type="submit" disabled={busy}>{busy ? 'Sending…' : 'Send my confirmation link'}</ActionButton></fieldset> : null}
