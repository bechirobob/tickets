...
export default async function HostsPage() {
  ...
  `).all<HostRow>();
  return <main className="hosts-page"><header className="directory-header"><Link href="/" aria-label="Back to home"><ArrowLeft size={16} /><span className="directory-header__back-label">Home</span></Link><Link href="/" className="brand-mark"><BrandLogo /></Link><PublicNavigation /></header><section className="directory-intro"><p className="eyebrow">Hosts</p><h1>Meet the usual instigators.</h1><p>The people giving Accra somewhere to be. Meet the Hosts, see their nights and follow your favourites after your first verified ticket.</p></section><section className="hosts-list">{!hosts.results.length ? <p>Host profiles are coming soon. The parties are already in The Drop.</p> : null}{hosts.results.map((host) => <article key={host.slug}><div className="host-monogram">{host.name.split(/\s+/u).map((word) => word[0]).join("").slice(0, 2)}</div><div><p>{host.verificationStatus === "verified" ? <><BadgeCheck size={13} /> Verified Host</> : "Reviewed Host"}</p><h2>{host.name}</h2><span>{host.city} · {host.eventCount} {Number(host.eventCount) === 1 ? "night" : "nights"}</span><p>{host.bio}</p><ActionLink href={`/hosts/${host.slug}`} variant="text">View Host</ActionLink></div></article>)}</section></main>;
}
