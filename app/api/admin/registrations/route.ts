...
export async function POST(request: Request) {
  ...
  try {
    if (body.action === 'settings') {
      ...
      if (body.mode === 'rsvp' && (!registrationScheduleReady(s) || body.capacity < 1)) throw new Error('Confirm the event date, start time and admission capacity before opening RSVP.');
