...
async function findFlash(id: string) {
  return env.DB.prepare(`
    SELECT id, event_slug AS eventSlug, image_data AS imageData, content_type AS contentType,
           status, expires_at AS expiresAt
    FROM room_flashes WHERE id = ? AND status != 'deleted' AND expires_at > ? LIMIT 1
  `).bind(id, new Date().toISOString()).first<{ id: string; eventSlug: string; imageData: number[] | null; contentType: string; status: string; expiresAt: string }>();
}
...
export async function GET(request: Request, context: Context) {
  ...
  if (!flash.imageData) return new Response("Not found", { status: 404 });
  return new Response(Uint8Array.from(flash.imageData), { headers: { "content-type": flash.contentType, "cache-control": "private, no-store", "content-disposition": "inline; filename=moderation-flash.webp", "x-robots-tag": "noindex, noarchive, noimageindex" } });
}
...
export async function DELETE(request: Request, context: Context) {
  ...
  await env.DB.batch([
    env.DB.prepare(`UPDATE room_flashes SET image_data = NULL, status = 'deleted', moderation_result = 'moderator_removed', deleted_at = ? WHERE id = ?`).bind(now, flash.id),
