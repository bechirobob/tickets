...
export async function PATCH(request: Request) {
  ...
  try {
    const body = await request.json() as { tagline?: string; id?: string; action?: Action; note?: string; curationNote?: string; scheduledPublishAt?: string };
    ...
    const tagline = normalizeEventTagline(body.tagline ?? current.tagline, ["approve", "schedule", "publish"].includes(body.action));
