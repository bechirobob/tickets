...
function NotificationRow({ item, mark, now, onNavigate }: { item: NotificationItem; mark: Feed["mark"]; now: number; onNavigate?: () => void }) {
  ...
  const ticket = ["ticket_transfer", "purchase_confirmation", "registration_update"].includes(item.kind);
  ...
  return <article className={`buzz-row ${item.readAt ? "read" : "unread"}`}>
    ...
    <div className="buzz-row-main">
      <header><span>{host ? "Host announcement" : ticket ? "Booking" : "The Room"}{item.eventTitle ? ` · ${item.eventTitle}` : ""}</span><time dateTime={item.createdAt} title={stamp} aria-label={stamp}>{notificationTime(item.createdAt, now)}</time></header>
...
export default function NotificationFeed({ feed, compact = false, onNavigate }: { feed: Feed; compact?: boolean; onNavigate?: () => void }) {
  const [filter, setFilter] = useState<"all" | "unread" | "host">("all");
