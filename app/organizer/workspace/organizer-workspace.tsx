      ...
      {loading ? <div className="organizer-empty"><Loader2 className="spin" /> Loading your record…</div> : <>
        ...
        {data.events.length === 0 ? <div className="organizer-empty"><CalendarRange /><h2>No approved Nights yet.</h2><p>Your submission trail will stay above while the curation team reviews it.</p><Link href="/organizer/submit">Submit an event</Link></div> : <>
          ...
          {selected ? <section className="organizer-dashboard" id="organizer-event-detail">
            ...
            <RegistrationManager key={selected.slug} eventSlug={selected.slug} expanded /><details className="organizer-tools-fold"><summary>Event details, sales & operations</summary><div className="organizer-metrics"><article><TicketCheck /><small>Paid orders</small><b>{selected.paidOrders}</b></article><article><UsersRound /><small>Admissions issued</small><b>{selected.issuedAdmissions}</b></article><article><ScanLine /><small>Checked in</small><b>{selected.checkedInAdmissions}</b></article><article><CheckCircle2 /><small>Gross collected</small><b>{money(selected.grossMinor)}</b></article></div><div className="organizer-grid">
