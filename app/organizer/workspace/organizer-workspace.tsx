...
type WorkspaceData = { events: EventItem[]; tiers: Tier[]; settlements: Settlement[]; requests: RequestItem[]; gateStaff: GateStaff[]; attendeeAnswers: AttendeeAnswer[] };
...
export default function OrganizerWorkspace({ actor, role }: { actor: string; role: StaffRole }) {
  ...
  return <main className="organizer-workspace">
    ...
    {loading ? <div className="organizer-empty"><Loader2 className="spin" /> Loading your events…</div> : data.events.length === 0 ? <div className="organizer-empty"><CalendarRange /><h2>No assigned events yet.</h2><p>Ask a BeCore owner to connect your approved event to this account.</p><Link href="/organizer/submit">Submit an event</Link></div> : <>
      <div className="workspace-event-picker organizer-event-picker"><label htmlFor="organizer-event">Assigned Night</label><select id="organizer-event" value={selectedSlug} onChange={(event) => { setSelectedSlug(event.target.value); setMessage(""); }}>{data.events.map((item) => <option key={item.slug} value={item.slug}>{item.title}</option>)}</select><span>{selected ? `${new Date(selected.startsAt).toLocaleDateString("en-GH", { dateStyle: "medium" })} · ${selected.eventState.replaceAll("_", " ")}` : "Choose an assigned Night."}</span></div>
