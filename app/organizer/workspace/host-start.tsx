...
export default function HostStart({eventSlug,onOpen,active}:{eventSlug:string;onOpen:(view:string)=>void;active:boolean}) {
  ...
  const ended=Boolean(event&&Date.parse(event.endsAt)<asOf);
  const shareable=Boolean(event&&event.status==='published'&&!['cancelled','archived'].includes(event.eventState)&&!ended);
  ...
  return <section className="host-start" aria-label="Your event at a glance">
    ...
    {data&&event?<>
      <div className="host-start__numbers"><div><span>Confirmed RSVP guests</span><b>{data.summary.rsvp.totals.confirmedGuests}</b></div><div><span>Paid orders</span><b>{data.summary.sales.orders}</b></div><div><span>Ticket sales</span><b>{money(data.summary.sales.ticketSalesMinor)}</b></div><div><span>Checked in</span><b>{data.summary.checkedIn} <small>/ {data.summary.expected}</small></b></div></div>
      <p className="host-start__note">Ticket sales exclude booking fees. Payout statements are under Event & sales.</p>
      ...
      {!ended?<ul className="host-start__checks">{checks.map(c=><li key={c.label}><span aria-label={c.ready?'Ready':'Needs attention'}>{c.ready?<Check size={18}/>:<span className="host-start__dot"/>}</span><div><b>{c.label}</b><p>{c.detail}</p></div><button type="button" onClick={()=>onOpen(c.view)}>{c.ready?'Review':'Set up'}<span className="sr-only"> {c.label}</span><ArrowRight size={16}/></button></li>)}</ul>:null}
      <div className="host-start__share"><div><h4>{shareable?'Your link, ready to go':ended?'This event has ended':'Your public link is waiting'}</h4><p>{shareable?event.accepting===0?'New registrations are paused. Review Guest setup before sharing.':event.closesAt&&Date.parse(event.closesAt)<=asOf?'Registration has closed. The page is still available.':event.scheduleStatus!=='confirmed'?'Dates are coming soon. Check your guest setup before sharing.':'Open the page, check the details, then share it.':ended?'Open analytics for the full breakdown.':event.eventState==='cancelled'?'This event is cancelled.':'BeCore will publish the event after review. You can prepare the details above.'}</p>{shareable&&event.closesAt?<small>Registration deadline: {time(event.closesAt)} (Accra).</small>:null}</div>
