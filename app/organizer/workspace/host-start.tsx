...
export default function HostStart({eventSlug,onOpen,active,children}:{eventSlug:string;onOpen:(view:string)=>void;active:boolean;children?:ReactNode}) {
  ...
  return <><section className="host-start" aria-label="Your event at a glance">
    ...
    {data&&event?<>
      ...
      {shareable?<><label htmlFor={`share-${eventSlug}`}>Guest link<input id={`share-${eventSlug}`} value={url} readOnly onFocus={e=>e.target.select()}/></label><div className="host-start__actions"><button type="button" onClick={()=>void copy()}><Copy size={16}/>Copy guest link</button><Link href={url} target="_blank" rel="noopener noreferrer">Open guest page <ArrowRight size={16}/></Link></div></>:null}
      <button type="button" onClick={()=>onOpen("insights")}>View analytics & tracked links <ArrowRight size={16}/></button></div>
