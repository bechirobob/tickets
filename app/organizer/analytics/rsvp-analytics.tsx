...
export default function RsvpReport({ data, eventSlug }: { data: RsvpAnalytics; eventSlug: string }) {
  ...
  return <section className="rsvp-report" aria-labelledby="rsvp-report-title">
    ...
    <details className="rsvp-report__links"><summary>Share a link. See what it brings.</summary>{eventSlug === 'all' ? <p>Choose a Night above to get its tracked RSVP links.</p> : <div><label>Where you’ll share it<select value={selected} onChange={event => { setSource(event.target.value); setNotice(''); }}>{options.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label>RSVP link<input readOnly value={link} onFocus={event => event.target.select()} /></label><button type="button" onClick={() => void copy()}>Copy link</button><p role="status">{notice}</p></div>}</details>
  </section>;
}
