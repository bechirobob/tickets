...
export default function PromoterOperations({ actor, role }: { actor: string; role: StaffRole }) {
  ...
  const load = useCallback(async () => { const response = await operationsFetch("/api/admin/promoters", { cache: "no-store" }); const data = await response.json() as { events?: Array<{ slug: string; title: string }>; codes?: Code[]; error?: string }; if (response.ok) { setEvents(data.events ?? []); setCodes(data.codes ?? []); setEventSlug((current) => current || data.events?.[0]?.slug || ""); } else setNotice(data.error ?? "Promoter links could not load."); }, []);
  useEffect(() => {
    operationsFetch("/api/admin/promoters", { cache: "no-store" })
