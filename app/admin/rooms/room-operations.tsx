...
export default function RoomOperations({ actor, role }: { actor: string; role: StaffRole }) {
  ...
  return <main className="room-ops">
    ...
    <section>
      <header><div><p>Guest conversations</p><h1>Room + Flashes</h1></div><button onClick={load}><RefreshCw size={14} /> Refresh reports</button></header>
      ...
      <div className="room-ops__control-grid">
        <article className="room-ops__panel"><ShieldPause /><p className="eyebrow">Conversation</p><h2>Chat settings</h2><div className="room-ops__compact-controls"><label>Slow mode<select value={slowModeSeconds} onChange={(event) => setSlowModeSeconds(Number(event.target.value))}><option value="0">Off</option><option value="5">5 seconds</option><option value="15">15 seconds</option><option value="30">30 seconds</option></select></label><label className="room-ops__check"><input type="checkbox" checked={emergencyReadOnly} onChange={(event) => setEmergencyReadOnly(event.target.checked)} /> Pause guest messages</label><label className="room-ops__check"><input type="checkbox" checked={archived} onChange={(event) => setArchived(event.target.checked)} /> Archive Room</label><button onClick={() => operate({ action: "settings", slowModeSeconds, emergencyReadOnly, archived }, "Room controls saved.")} disabled={working || !eventSlug}>Apply controls</button><button onClick={() => operate({ action: "clear_pin" }, "Pinned announcement cleared.")} disabled={working || !eventSlug}>Clear pin</button></div></article>
        <article className="room-ops__panel"><Megaphone /><p className="eyebrow">Host update</p><h2>Post an update</h2><p>Published to eligible guests’ inboxes and devices with host alerts enabled.</p><label>Announcement<textarea maxLength={500} value={announcement} onChange={(event) => setAnnouncement(event.target.value)} placeholder="Entry update, set time or useful event notice" /></label><label className="room-ops__check"><input type="checkbox" checked={pinned} onChange={(event) => setPinned(event.target.checked)} /> Pin above chat</label><button onClick={publish} disabled={working || !announcement.trim() || !eventSlug}>Publish update</button></article>
        <article className="room-ops__panel"><Image /><p className="eyebrow">Official memory</p><h2>After the event</h2><label>Title<input maxLength={120} value={memoryTitle} onChange={(event) => setMemoryTitle(event.target.value)} placeholder="The night, officially" /></label><label>Note<textarea maxLength={800} value={memoryBody} onChange={(event) => setMemoryBody(event.target.value)} placeholder="A short post-night note. Event artwork is attached." /></label><button onClick={publishMemory} disabled={working || !memoryTitle.trim() || !memoryBody.trim() || !eventSlug}>Publish memory</button></article>
      </div>
      <section className="room-ops__moderation">
        <header><div><p className="eyebrow">Moderation</p><h2>Reports & suspensions</h2></div><span>{selectedReports.length} messages · {selectedFlashReports.length} Flashes · {selectedSuspensions.length} suspended</span></header>
        <div>
          <article className="room-ops__reports"><header><div><Flag /><span><b>{selectedReports.filter((report) => report.status === "open").length}</b> Messages</span></div></header>{selectedReports.length === 0 ? <p className="room-ops__empty">No message reports.</p> : selectedReports.map((report) => <section key={report.id}><div><span>{report.reason}</span><time>{new Date(report.createdAt).toLocaleString("en-GH")}</time></div><blockquote>{report.message?.deletedAt ? "Message already removed" : report.message?.content ?? "Message unavailable"}</blockquote><p>{report.message?.displayName ?? "Unknown attendee"}</p>{report.details && <small>Report details: {report.details}</small>}{report.status === "open" && report.message && !report.message.deletedAt && <div className="room-ops__row"><button onClick={() => remove(report)}><Trash2 size={13} /> Remove</button><button onClick={() => operate({ action: "suspend", attendeeId: report.message?.attendeeId, reason: `Report: ${report.reason}` }, "Attendee suspended from this Room.")}><UserRoundX size={13} /> Suspend</button></div>}</section>)}</article>
          <article className="room-ops__reports room-ops__flashes"><header><div><Camera /><span><b>{selectedFlashReports.filter((report) => report.status === "open").length}</b> Flashes</span></div></header>{selectedFlashReports.length === 0 ? <p className="room-ops__empty">No reported Flashes.</p> : selectedFlashReports.map((report) => <section key={report.id}><div><span>{report.reason}</span><time>{new Date(report.createdAt).toLocaleString("en-GH")}</time></div>{report.flashStatus !== "deleted" && <img src={`/api/admin/rooms/flashes/${encodeURIComponent(report.flashId)}`} alt={`Reported Flash from ${report.displayName}`} />}<p>{report.displayName}</p>{report.details && <small>Report details: {report.details}</small>}{report.status === "open" && report.flashStatus !== "deleted" && <button onClick={() => removeFlash(report)}><Trash2 size={13} /> Remove permanently</button>}</section>)}</article>
