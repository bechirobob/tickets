...
import { Flag, Megaphone, RefreshCw, Trash2 } from "lucide-react";
...
export default function RoomOperations({ actor, role }: { actor: string; role: StaffRole }) {
  ...
  const load = useCallback(async () => {
    ...
    const result = await response.json() as { reports?: Report[]; events?: { slug: string; title: string }[]; error?: string };
    if (response.ok) { setReports(result.reports ?? []); setEvents(result.events ?? []); setEventSlug((current) => current || result.events?.[0]?.slug || ""); } else setNotice(result.error ?? "Reports could not be loaded.");
  }, []);
  useEffect(() => {
    fetch("/api/admin/rooms", { cache: "no-store" })
      .then(async (response) => ({ response, result: await response.json() as { reports?: Report[]; events?: { slug: string; title: string }[]; error?: string } }))
      .then(({ response, result }) => {
        if (response.ok) { setReports(result.reports ?? []); setEvents(result.events ?? []); setEventSlug(result.events?.[0]?.slug || ""); }
