  ...
  return <main className="room-ops">
    ...
    <section>
      ...
      <div className="room-ops__grid">
        <article className="room-ops__announce"><Megaphone /><p className="eyebrow">Room controls</p><h2>Speak, pace or pause.</h2><label>Event<select value={eventSlug} onChange={(event) => setEventSlug(event.target.value)}>{events.map((item) => <option key={item.slug} value={item.slug}>{item.title}</option>)}</select></label><div className="room-ops__compact-controls"><label>Slow mode<select value={slowModeSeconds} onChange={(event) => setSlowModeSeconds(Number(event.target.value))}><option value="0">Off</option><option value="5">5 seconds</option><option value="15">15 seconds</option><option value="30">30 seconds</option></select></label><label className="room-ops__check"><input type="checkbox" checked={emergencyReadOnly} onChange={(event) => setEmergencyReadOnly(event.target.checked)} /> Emergency read-only</label><label className="room-ops__check"><input type="checkbox" checked={archived} onChange={(event) => setArchived(event.target.checked)} /> Archive Room</label><button onClick={() => operate({ action: "settings", slowModeSeconds, emergencyReadOnly, archived }, "Room controls saved.")} disabled={working}><ShieldPause size={14} /> Apply</button><button onClick={() => operate({ action: "clear_pin" }, "Pinned announcement cleared.")} disabled={working}>Clear pin</button></div><label>Announcement<textarea maxLength={500} value={announcement} onChange={(event) => setAnnouncement(event.target.value)} placeholder="Entry update, set time or useful event notice" /></label><label className="room-ops__check"><input type="checkbox" checked={pinned} onChange={(event) => setPinned(event.target.checked)} /> Pin above chat</label><button onClick={publish} disabled={working || !announcement.trim() || !eventSlug}>Publish to The Room</button><hr /><p className="eyebrow"><Image size={13} /> Official memory</p><label>Title<input maxLength={120} value={memoryTitle} onChange={(event) => setMemoryTitle(event.target.value)} placeholder="The night, officially" /></label><label>Note<textarea maxLength={800} value={memoryBody} onChange={(event) => setMemoryBody(event.target.value)} placeholder="A short post-night note. Event artwork is attached." /></label><button onClick={publishMemory} disabled={working || !memoryTitle.trim() || !memoryBody.trim()}>Publish memory</button></article>
        <article className="room-ops__reports"><header><div><Flag /><span><b>{reports.filter((report) => report.status === "open").length}</b> open reports</span></div></header>{reports.length === 0 ? <p className="room-ops__empty">No attendee reports. Quiet is good when it is real.</p> : reports.map((report) => <section key={report.id}><div><span>{report.reason}</span><time>{new Date(report.createdAt).toLocaleString("en-GH")}</time></div><blockquote>{report.message?.deletedAt ? "Message already removed" : report.message?.content ?? "Message unavailable"}</blockquote><p>{report.message?.displayName ?? "Unknown attendee"} · {report.eventSlug}</p>{report.details && <small>Reporter context: {report.details}</small>}{report.status === "open" && report.message && !report.message.deletedAt && <div className="room-ops__row"><button onClick={() => remove(report)}><Trash2 size={13} /> Remove</button><button onClick={() => operate({ action: "suspend", attendeeId: report.message?.attendeeId, reason: `Report: ${report.reason}` }, "Attendee suspended from this Room.")}><UserRoundX size={13} /> Suspend</button></div>}</section>)}</article>
        <article className="room-ops__reports room-ops__flashes"><header><div><Camera /><span><b>{flashReports.filter((report) => report.status === "open").length}</b> Flash reports</span></div></header>{flashReports.length === 0 ? <p className="room-ops__empty">No reported Flashes.</p> : flashReports.map((report) => <section key={report.id}><div><span>{report.reason}</span><time>{new Date(report.createdAt).toLocaleString("en-GH")}</time></div>{report.flashStatus !== "deleted" && <img src={`/api/admin/rooms/flashes/${encodeURIComponent(report.flashId)}`} alt={`Reported Flash from ${report.displayName}`} />}<p>{report.displayName} · {report.eventSlug}</p>{report.details && <small>Reporter context: {report.details}</small>}{report.status === "open" && report.flashStatus !== "deleted" && <button onClick={() => removeFlash(report)}><Trash2 size={13} /> Remove Flash permanently</button>}</section>)}{suspensions.filter((item) => item.eventSlug === eventSlug).map((item) => <section key={item.attendeeId}><div><span>Suspended</span><time>{new Date(item.suspendedAt).toLocaleString("en-GH")}</time></div><p>{item.displayName}</p><small>{item.reason}</small><button onClick={() => operate({ action: "restore", attendeeId: item.attendeeId }, "Attendee restored to this Room.")}><UserRoundCheck size={13} /> Restore</button></section>)}</article>
      </div>
    </section>
  </main>;
}
