...
export default function OrderOperations({ actor, role }: { actor: string; role: StaffRole }) {
  ...
  const load = useCallback(async () => {
    ...
    const data = await response.json() as { orders?: Order[]; reconciliationRuns?: Run[]; settlements?: Settlement[]; disputes?: Dispute[]; error?: string };
  ...
  useEffect(() => {
    fetch("/api/admin/orders", { cache: "no-store" })
      .then(async (response) => ({ response, data: await response.json() as { orders?: Order[]; reconciliationRuns?: Run[]; settlements?: Settlement[]; disputes?: Dispute[]; error?: string } }))
      .then(({ response, data }) => {
  ...
  async function operate(action: string, order?: Order) {
    const key = `${action}:${order?.id ?? "all"}`; setWorking(key); setMessage("");
    let reason = "";
    if (action === "refund") { reason = window.prompt("Why is this full refund being issued?")?.trim() ?? ""; if (!reason) { setWorking(""); return; } }
    const response = await fetch("/api/admin/orders", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, orderId: order?.id, reason }) });
