...
export default function EventOperationsHub({ actor, role }: { actor: string; role: StaffRole }) {
  ...
  return <main className="ops-page"><OperationsNav actor={actor} role={role} active="/admin/operations" /><section className="ops-main operations-hub"><header><div><p>{role === "finance" ? "Money without guesswork" : "Live truth, one page"}</p><h1>{role === "finance" ? "Finance overview" : "Event operations"}</h1></div><button onClick={() => void load()}><RefreshCw size={14} /> Refresh</button></header>
    ...
    {event && metric ? <><section className="operations-title"><div><p>{event.venue}</p><h2>{event.title}</h2></div>{canEvents ? <strong className={readiness === 100 ? "ready" : ""}>{readiness}% ready</strong> : null}</section>
      ...
      <section><header><Users /><div><b>High-risk approvals</b><span>Only your role&apos;s approvals appear. Requester and approver must be different people.</span></div></header>{data.approvals.filter((item) => !item.event_slug || item.event_slug === selected).map((item) => <article key={item.id}><span><b>{item.kind.replaceAll("_", " ")}</b><small>{item.requested_by_email} · {item.status}{item.failure_reason ? ` · ${item.failure_reason}` : ""}</small></span>{item.status === "pending" ? <div><button onClick={() => void act({ action: "approval", approvalId: item.id, decision: "approve" })}>Approve</button><button onClick={() => void act({ action: "approval", approvalId: item.id, decision: "reject" })}>Reject</button></div> : null}</article>)}{canEvents ? <button className="operations-danger" onClick={() => protectedAction("request_cancellation")}>Request cancellation</button> : null}{canFinance ? <button className="operations-danger" onClick={() => protectedAction("request_mass_refund")}>Request mass refund</button> : null}</section></div>
