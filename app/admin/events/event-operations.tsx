...
type Tier = { id?: string; code: string; name: string; description: string; priceMinor: number; admissionsPerUnit: number; capacityAdmissions: number; maxUnitsPerOrder: number; status: "available" | "sold_out" | "hidden"; salesOpenAt: string | null; salesCloseAt: string | null; roomBadge: "VIP" | null; allocatedAdmissions?: number };
type EventRecord = { slug: string; title: string; venue: string; venueMapUrl: string; area: string; startsAt: string; endsAt: string; vibe: string; salesOpenAt: string | null; salesCloseAt: string | null; ageRestriction: string; lineup: string; eventState: string; isTestEvent: boolean; rescheduledFrom: string | null; curationNote: string; tagline?: string | null; dressCode?: string | null; colourScheme?: string | null; awarenessNote?: string | null; guestPerk?: string | null; status: string; tiers: Tier[] };
...
export default function EventOperations({ actor, role }: { actor: string; role: StaffRole }) {
  ...
  async function saveCopy() {
    ...
    try {
      const response = await fetch("/api/admin/events", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ slug: selected.slug, action: "save_copy", tagline: selected.tagline }) });
