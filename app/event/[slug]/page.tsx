  ...
  return <main className="event-page compact-event-page poster-event-page" data-colour-scheme={colourScheme} style={eventPresentationStyle(event)}>
    ...
    <div className="event-detail-layout">
      ...
      <aside className="compact-ticket-panel" id="register">
        {registration && registrationMode !== "paid" ? <><p className="eyebrow">{registrationMode === "interest" ? "Stay in the loop" : "RSVP"}</p>{registrationsOpen(registration) ? <RegistrationForm eventSlug={slug} mode={registrationMode as "rsvp" | "interest"} maxPartySize={registration.maxPartySize} approvalRequired={Boolean(registration.approvalRequired)} initiallyOpen={query.register === "1"} deadline={registration.closesAt ?? undefined} /> : <p>The guest list is closed for this one.</p>}</> : <>
        ...
        {salesPending ? <p className="event-state-notice">Ticket sales open soon.</p> : event.eventState === "cancelled" ? <p className="event-state-notice">This one’s been called off. Already paid? Check My Nights for refund help.</p> : event.eventState === "postponed" ? <p className="event-state-notice">New date loading. Hold onto your ticket and check back for the host’s update.</p> : registration && !registrationsOpen(registration) ? <p className="event-state-notice">The guest list is closed for this one.</p> : available ? <ActionLink href={`/checkout/${slug}${promoterCode ? `?ref=${encodeURIComponent(promoterCode)}` : ""}`} className="checkout-link" icon={<Ticket size={18} />}>Get tickets</ActionLink> : <span className="checkout-link checkout-link--disabled">Tickets are taking a breather</span>}
