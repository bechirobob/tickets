...
export default function FlashViewer({ slug, flash, onClose, onOpened, onReport, onRemove }: {
  ...
  return <RoomOverlay label={`Flash from ${flash.mine ? "you" : flash.displayName}`} className="room-overlay--immersive" onClose={onClose} beforeClose={endSession}>{(dismiss) => <section className="flash-stage flash-viewer">
    ...
    <header className="flash-stage__header"><span><b>{flash.mine ? "Your Flash" : flash.displayName}</b><small>{flash.mine ? "Your own preview" : "One look. Make it count."}</small></span><button type="button" aria-label="Close Flash" onClick={dismiss}><X aria-hidden="true" size={22} /></button></header>
    ...
    {!image && <div className="flash-stage__waiting"><Aperture aria-hidden="true" size={30} /><p role={error ? "alert" : "status"}>{error || "Opening your Flash…"}</p>{error && !gone && <button type="button" onClick={() => { setImage(null); setAttempt((value) => value + 1); }}>Try opening again</button>}</div>}
    <footer className="flash-stage__footer flash-viewer__footer"><small>{flash.mine ? "Only you can preview it again." : "Gone when you leave or the timer ends."}</small><button type="button" onClick={() => { endSession(); if (flash.mine) onRemove(); else onReport(); }} aria-label={flash.mine ? "Remove your Flash" : "Report this Flash"}>{flash.mine ? <Trash2 aria-hidden="true" size={17} /> : <Flag aria-hidden="true" size={17} />}{flash.mine ? "Remove" : "Report"}</button></footer>
  </section>}</RoomOverlay>;
}
