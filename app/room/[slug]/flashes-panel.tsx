...
export default function FlashesPanel({ slug, readOnly, expiresAt, refreshKey, onCount, onFlashes, galleryOpen,
  ...
  return <>
    {notice && <div className="room-media-notice" role="status"><span>{notice}</span><button type="button" aria-label="Dismiss Flash notice" onClick={() => setNotice("")}><X aria-hidden="true" size={16} /></button></div>}
    {galleryOpen && !cameraOpen && !active && !managing && <RoomOverlay label="Room Flashes" className="room-overlay--sheet" onClose={onGalleryClose}>{(dismiss) => <section className="room-sheet flash-inbox">
      <header className="room-sheet__header"><div><span className="room-surface-kicker"><Aperture aria-hidden="true" size={17} /> Flashes</span><h2>You had to be there.</h2><p>Ten seconds. One look. Zero camera-roll archaeology.</p></div><button type="button" aria-label="Close Flashes" onClick={dismiss}><X aria-hidden="true" size={20} /></button></header>
      <button type="button" className="flash-inbox__capture" onClick={() => setCameraOpen(true)}><Camera aria-hidden="true" size={22} /><span><b>Send a Flash</b><small>Show the Room what’s happening.</small></span><span aria-hidden="true">↗</span></button>
      ...
      {loading ? <p className="room-surface-empty" role="status">Finding the moments…</p> : loadError ? <div className="room-surface-empty"><p role="alert">{loadError}</p><button type="button" onClick={() => void load()}>Try again</button></div> : shown.length ? <ol className="flash-inbox__list">{shown.map((flash) => <li key={flash.id}>
        ...
        <button type="button" className="flash-inbox__more" aria-label={`Options for Flash from ${flash.mine ? "you" : flash.displayName}`} onClick={() => manage(flash)}><MoreHorizontal aria-hidden="true" size={18} /></button>
      </li>)}</ol> : <div className="room-surface-empty"><Aperture aria-hidden="true" size={28} /><h3>{filter === "unopened" && flashes.length ? "You’re all caught up." : "The camera’s waiting."}</h3><p>{flashes.length ? "The next moment is someone’s to send." : "The outfit. The entrance. The friend who said they weren’t coming."}</p></div>}
    ...
    {managing && <RoomOverlay label={managing.mine ? "Remove Flash" : "Flash options"} className="room-overlay--sheet" busy={busy} onClose={() => setManaging(null)}>{(dismiss) => <section className="room-sheet flash-manage">
      <header className="room-sheet__header"><div><span className="room-surface-kicker">{managing.mine ? <Trash2 aria-hidden="true" size={17} /> : <Flag aria-hidden="true" size={17} />} {managing.mine ? "Your Flash" : `From ${managing.displayName}`}</span><h2>{managing.mine ? "Delete this Flash for good?" : reporting ? "Tell us what happened." : "Keep the Room good."}</h2></div><button type="button" aria-label="Close Flash options" disabled={busy} onClick={dismiss}><X aria-hidden="true" size={20} /></button></header>
      {managing.mine ? <p>It disappears for everyone. This cannot be undone.</p> : !reporting ? <button type="button" className="room-surface-choice" onClick={() => setReporting(true)}><Flag aria-hidden="true" size={18} /><span><b>Report this Flash</b><small>Only the moderation team sees your report.</small></span></button> : <>
